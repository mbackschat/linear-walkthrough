...
>
> **Syntax highlighting:** All code blocks in the walkthrough MUST have a language identifier for syntax highlighting. Never use generic identifiers like `output` or `text` — always use the actual language. When a command extracts lines from a source file (e.g., `sed -n '10,20p' foo.py`), the output IS that language — tag it accordingly (e.g., `python`). Infer the language from the file extension in the command or from the file being read. Always use the most widely compatible language keys — use short, common identifiers (e.g., `python` not `python3`, `bash` not `shell`, `js` not `javascript`, `ts` not `typescript`, `rb` not `ruby`, `yml` not `yaml`). If the file extension is ambiguous or unavailable, guess the language from the content.
>
> **Markdown quality:**
